...
using namespace ::std;
#define JNI_API_DEF(f) Java_com_jamesfchen_image_gif_GifPlayer_##f
 ...
 *  来自网络的地址，来自sdcard的地址
 */
extern "C" JNIEXPORT jlong JNICALL JNI_API_DEF(setDataSource)(JNIEnv *env, jobject jgifplayer,jstring juriPath) {
...
extern "C" JNIEXPORT jint JNICALL JNI_API_DEF(getGifWidth)(JNIEnv *env, jobject jgifplayer,
...
extern "C" JNIEXPORT jint JNICALL JNI_API_DEF(getGifHeight)(JNIEnv *env, jobject jgifplayer,
                                                  jlong nativePlayerAddress) {
    GifPlayer *gifPlayer = (GifPlayer *) nativePlayerAddress;
    if (gifPlayer == nullptr) {
        LOGE(MODULE_NAME, "jgifplayer must not be null");
...
extern "C" JNIEXPORT void JNICALL JNI_API_DEF(start)(JNIEnv *env, jobject jgifplayer,jlong nativePlayerAddress) {
...
int register_com_hawksjamesf_image_DisplayEventReceiver(JNIEnv *env) {
    jclass cl = env->FindClass("com/jamesfchen/image/gif/GifPlayer");
    if ((env->RegisterNatives(cl, gifplayer_method_map,
                              sizeof(gifplayer_method_map) / sizeof(gifplayer_method_map[0]))) <
        0) {
        return -1;
    }
    return 0;
