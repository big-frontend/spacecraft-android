...
if [ $current_dir != "newLayoutExprimental" ];then
...
function run_test_and_systrace {
  test_method=$1
  ${ADB} -s ${DEVICE_ID} shell am instrument -w -r -e debug false -e class com.hawksjamesf.common.perf.PerformanceTest\#${test_method} com.hawksjamesf.common.test/androidx.test.runner.AndroidJUnitRunner &
  adb_pid=$!
