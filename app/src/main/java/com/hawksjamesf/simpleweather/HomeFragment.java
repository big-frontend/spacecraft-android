...
public class HomeFragment extends Fragment {
    ...
    me.zhouzhuo.zzweatherview.ZzWeatherView mWeatherView;
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        //画折线
        mWeatherView.setLineType(ZzWeatherView.LINE_TYPE_DISCOUNT);
        //画曲线(不够圆滑希望有更好的建议联系我(Email:admin@zhouzhuo810.me))
        //mWeatherView.setLineType(ZzWeatherView.LINE_TYPE_CURVE);

        //设置线宽
        mWeatherView.setLineWidth(6f);


        //设置一屏幕显示几列(最少3列)
        try {
            mWeatherView.setColumnNumber(5);
        } catch (Exception e) {
            e.printStackTrace();
        }

        //设置白天和晚上线条的颜色
        mWeatherView.setDayAndNightLineColor(Color.BLUE, Color.RED);


        //点击某一列
        mWeatherView.setOnWeatherItemClickListener(new ZzWeatherView.OnWeatherItemClickListener() {
            @Override
