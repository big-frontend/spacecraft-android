...
//    private static final String MAPPING_UPLOAD_URL = 'https://api.bugly.qq.com/openapi/file/upload/map'
    ...
    private boolean post(String url, HttpEntity httpEntity) {
        ...
        try {
            ...
            if (HttpStatus.SC_OK == statusCode) {
                ...
                project.logger.warn("Failed to execute POST for ${response.getStatusLine()}")
                return false
            }
    ...
    private String toUploadUrl(UploadInfo uploadInfo, boolean isMapping) {
        ...
        stringBuilder.append("?app_id=${uploadInfo.appId}")
        stringBuilder.append("&app_key=${uploadInfo.appKey}")
    ...
    private boolean uploadSymtabFile(UploadInfo uploadInfo, UploadFile uploadFile) {
        ...
        .addTextBody("app_id",uploadInfo.appId)
        .addTextBody("app_key",uploadInfo.appKey)
        .addTextBody("api_version","1")
        .addTextBody("channel","huawei")
        .addTextBody("symbolType",uploadFile.isMapping?"1":"3")
        .addTextBody("bundleId",uploadInfo.packageName)
        .addTextBody("productVersion",uploadInfo.version)
        .addTextBody("fileName",uploadInfo.uploadFileName)
        .addBinaryBody("file",new File(uploadFile.file.getAbsoluteFile()))
    ...
    private Task createUploadTask(Object variant) {
        ...
        def buglyTask = project.task("upload${variantName}SymtabFile") {
            doLast {
                ...
                println "BuglyPlugin upload task ${mappingFiles}"
                // Get symbol log file.
    ...
    void apply(Project project) {
        println "BuglyPlugin apply start"
        // Set colors of log
