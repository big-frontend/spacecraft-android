...
import static com.electrolytej.bundle2.util.LbsUtils.TAG_service;
...
//        locationManager.requestLocationUpdates(minTimes,minDistance,createCriteria(),createPendingIntent());
        locationManager.requestSingleUpdate(LocationManager.GPS_PROVIDER, new LocationListener() {
            ...
            public void onLocationChanged(Location location) {
                boolean b = Looper.getMainLooper().getThread() == Thread.currentThread();
                Log.d(TAG_service, "onStartJob sindle onLocationChanged: Is it main thread ? " + b);
                Log.d(TAG_service, "onStartJob sindle onLocationChanged: location" + location.getLatitude() +" , "+location.getLongitude());
    ...
    public boolean onStopJob(JobParameters jobParameters) {
        boolean b = Looper.getMainLooper().getThread() == Thread.currentThread();
        Log.d(TAG_service, "onStopJob: Is it main thread ? " + b);
        return true;
    }
